...
public class MineProxyHandler extends Thread {
	...
	public void run() {
		...
		Matcher dl_technic_url_1 = MineProxy.DL_TECHNIC_URL_1.matcher(url);
		Matcher dl_technic_url_2 = MineProxy.DL_TECHNIC_URL_2.matcher(url);
		...
		// If Version Request
			...
			String updateInfo = new String(SimpleRequest.get("http://" + MineProxy.authServer + "/update/client/0"));
		...
		// If Check Server Request
			...
			data = getRequest(url);
		} else if(dl_technic_url_1.matches() || dl_technic_url_2.matches()) {
			data = getRequest(url);
		}
